						...
						<form method="POST" action="" enctype="multipart/form-data">
							<div class="form-group row">
								...
								<div class="col-9 col-lg-10">
									<input id="inputEmail2" type="text" required="" placeholder="Brand Name" class="form-control" name="br_name" value="<?php if (isset($_GET['eid'])) {
																																							echo $BI['br_name'];
																																						}
																																						?>">
								</div>
								...
								<div class="col-9 col-lg-10">
									<select name="bigcat" class="form-control" required="">
										...
										if (isset($_GET['eid'])) {
											...
											echo "<option value='{$BI['bigcat_id']}
'>{$I2['bigcat_name']}
</option>";
										}
										...
										while ($CN = $res->fetch_assoc()) {
											echo "<option value='{$CN['bigcat_id']}
'>{$CN['bigcat_name']}
</option>";
										}
							...
							<table class="table">
								...
								<tbody>
									...
									while ($ref = $res->fetch_assoc()) {
										echo "<tr>";
										echo "<th scope='row'>{$ref['br_id']}
</th>";
										echo "<th scope='row'>{$ref['br_name']}
</th>";
										$BCI = $BC->get_cat($ref['bigcat_id']);
										echo "<th scope='row'>{$BCI['bigcat_name']}
</th>";
										echo "<th scope='row'>{$ref['br_img']}
</th>";
										if ($BCI['bigcat_sta']) {
											if ($ref['br_sta']) {
												echo "<td class='text-success'>Active</td>";
												echo "<td><a href='Br_man.php?eid={$ref['br_id']}
' class='btn btn-outline-warning'>Edit</a></td>";
												echo "<td><a href='Br_man.php?did={$ref['br_id']}
' class='btn btn-outline-danger'>Deactivate</a></td>";
											} else {
												echo "<td class='text-danger'>Brand Disactive</td>";
												echo "<td></td>";
												echo "<td><a href='Br_man.php?aid={$ref['br_id']}
