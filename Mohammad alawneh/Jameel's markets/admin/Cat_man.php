						...
						<form action="" method="POST" enctype="multipart/form-data">
							<div class="form-group row">
								...
								<div class="col-9 col-lg-10">
									<input id="inputEmail2" type="text" required="" placeholder="Name" class="form-control" name="name" value="<?php if (isset($_GET['Eid'])) {
																																					echo $I['bigcat_name'];
																																				}
																																				?>">
								</div>
					...
					<div class="card-body">
						<form action="" method="POST" enctype="multipart/form-data">
							<div class="form-group row">
								<label for="inputEmail2" class="col-3 col-lg-2 col-form-label text-right">Name</label>
								<div class="col-9 col-lg-10">
									<input id="inputEmail2" type="text" name="subname" required="" placeholder="Sub. Category name" class="form-control" value="<?php if (isset($_GET['eid'])) {
																																									echo $SI['subcat_name'];
																																								}
																																								?>">
								</div>
								...
								<div class="col-9 col-lg-10">
									<select id="inputEmail2" required="" name="bigcat" class="form-control">
										...
										if (isset($_GET['eid'])) {
											...
											echo "<option value='{$CTY['bigcat_id']}
'>{$CTY['bigcat_name']}
</option>";
										}
										...
										while ($CTN = $res->fetch_assoc()) {
											echo "<option value='{$CTN['bigcat_id']}
'>{$CTN['bigcat_name']}
</option>";
										}
						...
						<table class="table">
							...
							<tbody>
								...
								while ($CT = $res->fetch_assoc()) {
									echo "<tr>";
									echo "<th scope='row'>{$CT['bigcat_id']}
</th>";
									echo "<td>{$CT['bigcat_name']}
</td>";
									...
									if ($CT['bigcat_sta']) {
										...
										echo "<td><a href='Cat_man.php?Eid={$CT['bigcat_id']}
' class='btn btn-outline-warning'>Edit</a></td>";
										echo "<td><a href='Cat_man.php?Did={$CT['bigcat_id']}
' class='btn btn-outline-danger'>Deactivate</a></td>";
									} else {
										echo "<td class='text-danger'>Disactive</td>";
										echo "<td></td>";
										echo "<td><a href='Cat_man.php?aid={$CT['bigcat_id']}
' class='btn btn-outline-danger'>Active</a></td>";
									}
				...
				<div class="card-body">
					<table class="table">
						...
						<tbody>
							...
							while ($CT = $res->fetch_assoc()) {
								echo "<tr>";
								echo "<th scope='row'>{$CT['subcat_id']}
</th>";
								$RE = $CN->get_cat($CT['bigcat_id']);
								echo "<td>{$RE['bigcat_name']}
</td>";
								echo "<td>{$CT['subcat_name']}
</td>";
								if ($RE['bigcat_sta']) {
									if ($CT['subcat_sta']) {
										echo "<td class='text-success'>Active</td>";
										echo "<td><a href='Cat_man.php?eid={$CT['subcat_id']}
' class='btn btn-outline-warning'>Edit</a></td>";
										echo "<td><a href='Cat_man.php?did={$CT['subcat_id']}
' class='btn btn-outline-danger'>Deactivate</a></td>";
									} else {
										echo "<td class='text-danger'>Sub Category Disactive</td>";
										echo "<td></td>";
										echo "<td><a href='Cat_man.php?Aid={$CT['subcat_id']}
' class='btn btn-outline-danger'>Active</a></td>";
									}
								} else {
